...
Ort::Value GetOrtValueFromBMTensor(Tensor* const bmr_tensor) {
  ...
  [[maybe_unused]] ONNXTensorElementDataType type;
...
void ConvertOrtValueToBMTensor(Ort::Value &ort_value, Tensor* const bmr_tensor) {
  ...
  if (type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64 && strcmp(bm_tensor_type, "i32") == 0) {
    ...
    // std::free(temp_data_p);
  } else if (type == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64 && strcmp(bm_tensor_type, "fp32") ==0) {
    temp_data_p = malloc(bmr_tensor->num_elements()*ByteSize(bmr_tensor->tensor()->dtype));
