...
config = Config()
emotivoice_path = os.path.dirname(__file__)
bmodels_path = emotivoice_path + "/../../bmodels/emotivoice"
...
def get_models():
    ...
    tone_color_converter = ToneColorConverter(
        f"{bmodels_path}/converter/config.json", device=DEVICE
    )
    ...
    return (
        (style_encoder, generator, tokenizer, token2id, speaker2id),
...
if __name__ == "__main__":
    ...
    def predict(text_content, speaker, emotion, tgt_wav, agree):
        ...
        return (text_hint, src_wav, save_path)
            ...
            with gr.Row():
                with gr.Column():
                    speaker_gr = gr.Dropdown(
                        choices=speakers, label="Speaker ID (说话人)"
                    )
                ...
                with gr.Column():
                    ...
                    tts_button.click(
                        tts_only,
                        [content_gr, speaker_gr, emotion_gr],
                        outputs=[synthesize],
                    )
        ...
        with gr.Tab("TTS + Conversion mode"):
            with gr.Row():
                with gr.Column():
                    ...
                    emotion_gr = gr.Textbox(
                        label="语调情感 (开心/悲伤/愤怒/惊讶/冷酷...)"
                    )
                    ref_gr = gr.Audio(
                        label="目标音色",
                        # info="点击上传目标音色的音频文件",
                ...
                with gr.Column():
                    ...
                    tts_button.click(
                        predict,
                        [content_gr, speaker_gr, emotion_gr, ref_gr, tos_gr],
                        outputs=[out_text_gr, src_audio_gr, tgt_audio_gr],
                    )
        ...
        with gr.Tab("Conversion-only mode"):
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    cvt_button.click(
                        convert_only,
                        [cvt_src_gr, cvt_ref_gr, cvt_tos_gr],
                        outputs=[out_text_gr, cvt_audio_gr],
                    )
